...
const Gauge = ({ value=50, min=0, max=100, label, units, }) => {
    const backgroundArc = arc().innerRadius(0.65).outerRadius(1)
    ...
    const filledArc = arc().innerRadius(0.65).outerRadius(1)
    ...
    return (
        <div className="w-min text-center border border-gray-200 px-8 py-4 rounded-lg">
            <svg className="overflow-visible" width="9em" viewBox={[ -1, -1, 2, 1 ].join(" ")}>
                <defs>
            ...
            <div className="font-semibold text-3xl mt-5">{ format(",")(value) } {units}</div>
            {!!label && ( <div>{ label }</div> )}
        </div>
    )
}
